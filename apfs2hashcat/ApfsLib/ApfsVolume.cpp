...
int ApfsVolume::Init(paddr_t apsb_paddr, bool prompt_for_password)
{
	...
	if ((m_sb.apfs_fs_flags & 3) != APFS_FS_UNENCRYPTED)
	{
		...
		if (!m_container.GetVolumeKey(vek, m_sb.apfs_vol_uuid))
		{
			...
			std::cout << "Enter password for volume '"  << m_sb.apfs_volname << "': ";
...
bool ApfsVolume::GetPasswordHint(std::string &hint)
{
	return m_container.GetPasswordHint(hint, m_sb.apfs_vol_uuid);
}

bool ApfsVolume::IsEncrypted()
{
