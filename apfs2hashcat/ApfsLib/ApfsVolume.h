...
public:
	...
	int Init(paddr_t apsb_paddr, bool prompt_for_password=false);

	const char *name() const { return reinterpret_cast<const char *>(m_sb.apfs_volname); }
	const apfs_uuid_t &uuid() const { return reinterpret_cast<const apfs_uuid_t&>(m_sb.apfs_vol_uuid); }
	...
	bool GetPasswordHint(std::string &hint);
	bool IsEncrypted();
