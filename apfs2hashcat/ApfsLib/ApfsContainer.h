...
public:
	...
	ApfsVolume *GetVolume(unsigned int index, int &init_rc, const std::string &passphrase = std::string(), bool prompt_for_password=false);
	unsigned int GetVolumeCnt() const;
	bool GetVolumeInfo(unsigned int fsid, apfs_superblock_t &apsb);
	std::vector<kek_blob_t> GetVolumeKekBlobs(const apfs_uuid_t &vol_uuid) { return m_keymgr.GetValidKekBlobs(vol_uuid); };
