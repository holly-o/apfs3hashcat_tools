...
bool KeyManager::LoadRecsBag(Keybag &recs_bag, const apfs_uuid_t& volume_uuid)
{
	...
	return LoadKeybag(recs_bag, APFS_VOL_KEYBAG_OBJ, pr->pr_start_addr, pr->pr_block_count, volume_uuid);
}

std::vector<kek_blob_t> KeyManager::GetValidKekBlobs(const apfs_uuid_t& volume_uuid)
{
	std::vector<kek_blob_t> kekBlobs;
	Keybag recs_bag;

	if (!LoadRecsBag(recs_bag, volume_uuid))
		return kekBlobs;
...
bool KeyManager::GetVolumeKey(uint8_t* vek, const apfs_uuid_t& volume_uuid, const char* password)
{
	...
	std::vector<kek_blob_t> validKekBlobs = GetValidKekBlobs(volume_uuid);

	for (const kek_blob_t& kek_blob : validKekBlobs)
	{
